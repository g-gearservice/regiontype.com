...
지역을 타이핑으로 알리는 타자연습. v0.3.4 — 서울 25개 자치구와 구별 행정동 / 자유형.

## 0.3.4에서 바뀐 점

- 서울 25개 구마다 행정동 코스·지형(`data/*-dong.{course,geom}.json`). 서울 25곳 자치구 코스와 같이 고른다
- 타이틀은 로고와 메뉴만. 태그라인은 뺀다
- 설정은 야간 모드·효과음·애니메이션. 「정식 명칭 강제」는 없고 약칭은 항상 허용
- `/mimi/` — 미니 모터웨이즈 스타일 보드 (개발용 미리보기)
- 행정동은 `tools/build_dong.py` 한 번으로 찍는다. 옛 팔레트·슬롯 에셋과 그 빌드 스크립트는 지웠다
...
## 파일
    ...
    mimi/        미니 모터웨이즈 스타일 보드
    data/*.course.json   항목·별칭·한 줄 정보
    data/*.geom.json     화면 좌표로 투영된 SVG path
...
법정동·동명이지 대응, 순서형·위치형 모드, 서버 랭킹, 코스 에디터. 행정동 한 줄 소개는 강서구만 채워져 있다.
