...
한 사람이 한 줄을 차지한다 — 5분 판과 1분 판을 한 줄에 세우면 점수에 뜻이 없다.
    ...
    POST /dist     {c, t, who} → {bins, bucket, cap, total, score, over}
    POST /forget   {who} → {gone}   기록 코드가 가진 줄을 전부 내린다
...
계정이 없으므로 64비트 난수 하나가 순위표에서 내 줄을 갖는다. 순위표를 처음 쓸 때
만들어져 이 브라우저에만 남고, **화면에는 내보이지 않는다.** 사람을 가리키는 값이
아니라 줄을 가리키는 값이다. `crypto` 나 `localStorage` 가 막힌 자리에서는 순위 칸이
통째로 접힌다.

저장소를 비우면 그 줄로 돌아갈 길이 없다. 기기를 건너 잇는 손잡이는 두지 않았다 —
그걸 두려면 이 값을 사람이 옮겨 적을 수 있게 내보여야 하고, 그 순간 이 값은
비밀번호가 된다.
