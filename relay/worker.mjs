/* regiontype 중계기 — 정적 사이트가 혼자 못 하는 두 가지만 한다.
     ...
     POST /forget   기록 코드가 가진 줄을 전부 내린다
...
export function entry(c) {
  ...
  /* 줄의 주인은 이름이 아니라 이 값이다 — 이름으로 주인을 삼으면 남의 이름에
     높은 점수를 박아 그 사람이 영영 자기 기록을 못 올리게 만들 수 있다.
     브라우저가 한 번 만들어 두는 난수일 뿐, 사람을 가리키지 않는다. */
  const who = cut(c.who, 40);
  const [score, hits, tries] = [c.score, c.hits, c.tries].map(Number);
  const int = n => Number.isInteger(n) && n >= 0;
  const ok = w.ok && !!name && /^[a-z0-9]{8,40}$/.test(who)
...
/* 점수 분포. 남들이 어디쯤에 몰려 있고 내가 그 어디에 서는지를 한 장으로 본다.
   ...
   읽기인데 POST 인 이유는 기록 코드 때문이다. 자격증명을 주소줄에 실으면 중계·기록
   로그에 그대로 남는다. 내 점수는 순위표가 갖고 있으니 브라우저가 들고 다닐 필요도 없다. */
const BINS = 30;
async function dist(req, env, o) {
  ...
  const who = cut(c.who, 40);
  const me = /^[a-z0-9]{8,40}$/.test(who) ? who : null;
...
async function forget(req, env, o) {
  ...
  try { c = await req.json(); } catch { return reply(400, '읽을 수 없는 내용입니다.', o); }
  const who = cut(c.who, 40);
  if (!/^[a-z0-9]{8,40}$/.test(who)) return reply(400, '코드 모양이 맞지 않습니다.', o);
  const ok = await pass(env.RL_SC, ip(req));
  if (ok !== true) return shut(ok, o);
  return safely(o, async () => {
