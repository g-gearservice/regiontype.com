...
지역을 타이핑으로 알리는 타자연습. v0.3.9 (`VER=0.97`) — 나라별 행정구역, 화면 말 26개 언어.

## 0.97에서 바뀐 점

- 소개 페이지가 UI 언어를 따라 `about.{lang}.html` 로 열린다
- 설정 언어 목록 위·아래에 그라데이션 블러를 얹어 잘린 줄을 흐린다
- 커밋 메시지는 영어. README 는 영어·중국어·일본어·힌디어
...
## 파일
    ...
    about.html / about.*.html  소개 — 언어마다 한 장. style.css 토큰만 물려받고 app.js 는 부르지 않는다
    style.css
    ...
    data/i18n.json          화면 말 (26개 UI 언어). 손으로 쓴다
