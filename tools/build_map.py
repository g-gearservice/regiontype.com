"""GeoJSON → 화면 좌표. 서울만 다루므로 위도 보정한 등거리 투영으로 충분하다.

자치구는 도형이 아니라 도트 격자로 나간다. 격자 한 칸이 화면의 원 하나가 되고,
칸마다 그 자리를 차지한 자치구 번호가 들어간다. 정답을 맞히면 그 번호의 도트만
색이 찬다. 좌표계는 건물 타일·도로와 같은 1000 폭 공간을 쓴다.
"""
...
for row in grid:
    for v in row:
        if v is not None:
            counts[v] += 1
assert all(counts), f'도트를 못 받은 자치구: {[names[i] for i, c in enumerate(counts) if not c]}'
