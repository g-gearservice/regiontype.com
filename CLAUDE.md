...
릴리스 이름은 `vA.B.C` 세 자리다. **각 자리는 VER 에서 거꾸로 뽑는 값이 아니라,
바뀐 것의 성격으로 사람이 고른다:**

- **A** — 대규모 변경. 구조를 다시 짜거나 새 모드가 생기는 수준.
- **B** — 자잘한 변경. 평소 기능 추가·수정처럼 사용자 눈에 보이지만 A 만큼은
  아닌 것.
- **C** — 보안·정책·룰 관련 변경. 사용자 화면에는 안 보이는 것
  (`relay/` 토큰 처리, 레이트리밋, `CLAUDE.md`·커밋 규칙 같은 것).

상위 자리를 올리면 그 아래 자리는 0으로 되돌린다. 지금 값(`v0.3.9`)은 이 규칙이
생기기 전 것이라 그대로 두고, 다음 변경부터 이 기준으로 어느 자리를 올릴지
고른다.

`app.js` 의 `VER`(설정 화면 버전 줄에서 괄호 안 숫자)은 이 세 자리와 별개다 —
캐시 무효화용 내부 빌드 카운터라 고칠 때마다 그냥 `0.01` 씩 올린다
(`0.69 → 0.70`). `?v=` 는 줄이지 않는다.
